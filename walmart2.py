...
if is_headless():
    ...
    print("Will run in console mode instead.")
    HEADLESS = True
else:
    HEADLESS = False
...
if __name__ == "__main__":
    if HEADLESS:
        ...
        print("=" * 50)
        print("      WALMART SHOPPING SYSTEM (Console Mode)")
        print("=" * 50)
        print("Budget: $100.00 | Tax Rate: 10.44%")
        ...
        while True:
            ...
            if choice == '1':
                ...
                tax = subtotal * TAX_RATE
                total = subtotal + tax
                new_item_with_tax = item_price + (item_price * TAX_RATE)
                
                if (total + new_item_with_tax) > BUDGET:
                    remaining = BUDGET - total
                    print(f"Adding this item (${new_item_with_tax:.2f} with tax) would exceed your budget.")
                ...
                if not items:
                    ...
                    print(f"Tax ({TAX_RATE*100:.2f}%): ${tax:.2f}")
                ...
                if not os.path.isfile(TRANSACTIONS_FILE):
                    ...
                    for i, t in enumerate(transactions):
                        items_data = json.loads(t['items'])
                        print(f"{i+1}. {t['timestamp']} - {len(items_data)} items - ${float(t['total']):.2f}")
                    ...
                    while True:
                        view_details = input("\nView transaction details? (enter number or 'no'): ").strip()
                        if view_details.lower() in ['no', 'n']:
                            break
                        ...
                        try:
                            ...
                            if 0 <= idx < len(transactions):
                                ...
                                print("\n" + "="*50)
                                print(f"Transaction: {t['timestamp']}")
                                print("="*50)
                                
                                items_data = json.loads(t['items'])
                                print("Items:")
                                for i, item in enumerate(items_data):
                                    print(f"{i+1}. {item['name']} - ${float(item['price']):.2f}")
                                
                                print("-" * 50)
                                print(f"Subtotal: ${float(t['subtotal']):.2f}")
                                print(f"Tax: ${float(t['tax']):.2f}")
                                print(f"Total: ${float(t['total']):.2f}")
                                print("="*50)
                            else:
                ...
                print("\nThank you for using the Walmart Shopping System!")
                break
